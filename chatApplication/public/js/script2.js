...
ioGet.on("messageSent", (msg) => {
    ...
    if (msg.name == "you") {
        let change = document.getElementById("chatScreen").lastChild.firstChild;
        console.log(change);
        change.innerText = "you";
    }
    else {
        ...
        div1.textContent = msg.name; // 
        div2.textContent = msg.msg;
...
function addEventsListeners1() {
    ...
    createARoom.addEventListener("click", () => {
        ...
        ioGet.emit("createRoom", { roomName: inputRoomName, name: userName });;
    })
}
...
function addEventsListeners2() {
    ...
    joinARoom.addEventListener("click", () => {
        ...
        ioGet.emit("joinRoom", { roomName: inputRoomName, name: userName });
        // ioGet.emit("createOrJoin", joinRoomChoice.id);
...
document.addEventListener("keydown", (event) => {
    ...
    console.log("Display : ", five.style.display);

    if (key == "Enter" && five.style.display == "flex") {
